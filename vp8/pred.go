...
// interBModeProb are the default flat probabilities for B_PRED sub-mode
// decoding in inter-frames (vp8_bmode_prob in libvpx). Unlike keyframes
// which use context-dependent kf_bmode_prob[above][left], inter-frames
// use a single set of 9 probabilities for the bmode_tree.
var interBModeProb = [9]uint8{120, 90, 79, 133, 87, 85, 80, 111, 151}

// parsePredModeY4Inter decodes 4x4 sub-block prediction modes for INTRA
// B_PRED macroblocks in inter-frames. Uses flat probabilities (no context).
func (d *Decoder) parsePredModeY4Inter(mbx int) {
	prob := &interBModeProb
	for j := 0; j < 4; j++ {
		var p uint8
		for i := 0; i < 4; i++ {
