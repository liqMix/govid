...
// reconstructKeyframe handles keyframe macroblock reconstruction (original path).
func (d *Decoder) reconstructKeyframe(mbx, mby int, skip bool) bool {
...
// reconstructInterframe handles inter-frame macroblock reconstruction.
func (d *Decoder) reconstructInterframe(mbx, mby int, skip bool) bool {
	d.readInterMBModeAndMV(mbx, mby)

	if d.curRefFrame == 0 {
		// Intra MB in an inter-frame.
		d.prepareYBR(mbx, mby)
...
// clearResidualState zeros out the residual tracking state.
func (d *Decoder) clearResidualState(mbx int) {
	if d.usePredY16 {
		d.leftMB.nzY16 = 0
		d.upMB[mbx].nzY16 = 0
	}
...
// copyMBToImg copies the ybr workspace to the output image.
...
// applyInterResiduals applies DCT residuals on top of motion-compensated prediction.
// The existing inverseDCT functions already ADD to ybr values, so we can
// reuse them directly — the prediction is already in ybr from motion compensation.
func (d *Decoder) applyInterResiduals(mbx, mby int) {
	// Luma: WHT + per-4x4 DCT.
	for j := 0; j < 4; j++ {
		for i := 0; i < 4; i++ {
			n := 4*j + i
			y := 4*j + 1
