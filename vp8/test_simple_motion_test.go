...
func TestSimpleMotionMultiFrame(t *testing.T) {
	const webmPath = "testdata/simple_motion.webm"
	const refYUV = "testdata/simple_motion_frames.yuv"
	const numFrames = 5
	const w, h = 160, 120

	if _, err := os.Stat(webmPath); err != nil {
		t.Skip("simple_motion.webm not found")
	}
	...
	for i := 0; i < numFrames; i++ {
		...
		t.Logf("frame %d (%s): Y %d/%d wrong max=%d eof=%v", i, ft, wrongY, ySize, maxY, dec.fp.unexpectedEOF)
	}
}
