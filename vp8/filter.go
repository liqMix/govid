...
// computeInterFilterParam computes the loop filter parameters for an inter-frame
// macroblock, taking into account reference frame and mode deltas.
func (d *Decoder) computeInterFilterParam(mbx, mby int) filterParam {
	baseLevel := d.filterHeader.level
	if d.segmentHeader.useSegment {
		baseLevel = d.segmentHeader.filterStrength[d.segment]
		if d.segmentHeader.relativeDelta {
			baseLevel += d.filterHeader.level
		}
	}
	...
	if d.filterHeader.useLFDelta {
		...
		refIdx := int(d.curRefFrame)
		if refIdx < nRefLFDelta {
			level += d.filterHeader.refLFDelta[refIdx]
		}
		// Mode delta.
		if d.curRefFrame == 0 {
			// Intra MB.
			if !d.usePredY16 {
				level += d.filterHeader.modeLFDelta[0] // BPRED
			}
		} else {
			switch d.curMode {
			case interModeZEROMV:
				level += d.filterHeader.modeLFDelta[1]
			case interModeNEARESTMV, interModeNEARMV, interModeNEWMV:
				level += d.filterHeader.modeLFDelta[2]
			case interModeSPLITMV:
				level += d.filterHeader.modeLFDelta[3]
			}
		}
	}

	var p filterParam
	p.inner = d.curRefFrame != 0 || !d.usePredY16
