// Example: decode an AV1/MP4 video and save a frame as PNG.
//
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: av1-example <file.mp4>")
		os.Exit(1)
	}
	...
	fmt.Printf("Video: %dx%d @ %.2f fps, duration %v (codec: %s)\n",
		info.Width, info.Height, info.FrameRate, demuxer.Duration(), demuxer.CodecType())
