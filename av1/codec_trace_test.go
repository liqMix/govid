...
func TestTraceFirstSB(t *testing.T) {
	...
	state("init")
	// 1. Partition
	sym := sc.ReadSymbol(cdf.Partition[1][0], 10)
	t.Logf("Partition=%d", sym); state("post-partition")
	// 2. Skip
	sym = sc.ReadSymbol(cdf.Skip[0], 2)
	t.Logf("Skip=%d", sym); state("post-skip")
	// 3. CDEF
	v := sc.ReadLiteral(int(fh.CDEF.Bits))
	t.Logf("CDEF=%d", v); state("post-cdef")
	// 4. DeltaQ
	sym = sc.ReadSymbol(cdf.DeltaQ, 4)
	t.Logf("DeltaQ_base=%d", sym)
	dqAbs := sym
	if sym >= 3 {
		n := sc.ReadLiteral(3)
		ex := sc.ReadLiteral(int(n) + 1)
	...
	state("post-deltaq")
	// 5. Y mode
	sym = sc.ReadSymbol(cdf.KFIntraMode[0][0], 13)
	...
	if sym >= IntraVertical && sym <= IntraD67 {
		...
		ad := sc.ReadSymbol(cdf.AngleDelta[deltaIdx], 7)
		t.Logf("AngleDelta=%d (raw sym)", ad); state("post-angle")
	}
	// 7. UV mode
	uvSym := sc.ReadSymbol(cdf.IntraModeUVCfL[sym], 14)
	...
	if sym == IntraDC && fh.AllowScreenContentTools {
		...
		ps := sc.ReadSymbol(cdf.PaletteY[palCtx][0], 2)
	...
	for d := 0; d < maxDepth; d++ {
		...
		split := sc.ReadSymbol(cdf.TXSize[idx][0], 2)
		t.Logf("TXSplit[%d] idx=%d split=%d", d, idx, split)
		if split == 0 { break }
		txSize--
	}
	...
	if txSize < TX32x32 && !fh.ReducedTXSet {
		...
		ttSym := sc.ReadSymbol(cdf.IntraTXType2[txSqrCtx][modeCtx], 7)
